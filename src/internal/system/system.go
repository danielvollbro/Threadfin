...
import (
	...
	"threadfin/internal/host"
	htmlbuilder "threadfin/internal/html-builder"
	"threadfin/internal/plex"
	"threadfin/internal/provider"
	"threadfin/internal/settings"
	"threadfin/internal/ssdp"
...
func Init() (err error) {
	...
	// Default Logeinträge, wird später von denen aus der config.Settings.json überschrieben. Muss gemacht werden, damit die ersten Einträge auch im Log (webUI aangezeigt werden)
	config.Settings.LogEntriesRAM = 500

	// Variablen für den Update Prozess
	//config.System.Update.Git = "https://github.com/Threadfin/Threadfin/blob"
	...
	err = authentication.ActivatedSystemAuthentication(config.System.Folder.Config)
	if err != nil {
		return
	}
	...
	//config.System.Folder.Temp = config.System.Folder.Temp + config.Settings.UUID + string(os.PathSeparator)
